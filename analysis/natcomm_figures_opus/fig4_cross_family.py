...
def _panel_a(fig, k0: dict) -> None:
    ...
    ax.text(18.5, 7.9, r"6 shared seeds, $K\in\{0,0.08,0.15\}$",
            ha="center", va="center", fontsize=FS_TINY, color="#333333")
...
def _panel_d(fig, rows, inf: dict) -> dict:
    ...
    axk.scatter([0.430], [0.90], marker="D", c=INK, s=S_POINT, linewidths=0,
                clip_on=False)
    axk.text(0.451, 0.90, "mean, seed-bootstrap 95% CI (6 core seeds)", ha="left",
             va="center", fontsize=FS_TINY, color=MUTED)
    axk.scatter([0.012], [0.66], marker="^", facecolors="white",
                edgecolors=MUTED, linewidths=0.7, s=S_POINT, clip_on=False)
    axk.text(0.033, 0.66,
             "held-out seed (directional check; not in the confirmatory p-values)",
    ...
    axk.text(0.0, 0.42,
             ...
             ha="left", va="center", fontsize=FS_TINY, color=MUTED)
    axk.text(0.0, 0.18,
             "Supported: moments vs the histograms; centers vs intervals not established.",
