...
def build(_out_dir: Path | None = None) -> Path:
    ...
    panel_label(axE, "e", "Separation versus test–retest noise")
    ...
    axF.hist(null_tv, bins=np.linspace(np.min(null_tv), np.max(null_tv), 25), color="#8F969D", edgecolor="none")
    ...
    axF.text(np.max(null_tv) + 0.006, ytop * 0.99, "null",
