...
* **S11b** (e–h) — the prospective v1→v2 repair on the three Stage C v0.2
  ...
  calibration behind the Stage 3B gate.
...
so there is no gap. See ``SI_BRIEF.md``.
...
def _page_a() -> Path:
    ...
    # ------------------------------------------------------------------
    # a  training manifold in the two frozen gate descriptors
    # ------------------------------------------------------------------
    axA = mm_axes(fig, AX1_X, A_ROW1, AX_W, A_H1)
    panel_label_at(fig, COL1_X, A_ROW1 - 2.4, "a",
                   "Training manifold in fixed descriptors")
...
def _page_b() -> Path:
    ...
    # gate table to the right of panel h
    T_X = B_AX_X[0] + B_AX_H_W + 8.0
    fig_text_mm(fig, T_X, B_ROW2 + 3.2,
                "Stage 3B risk-ranking gate", fontsize=FS_SMALL, color=INK,
                ha="left", va="baseline", fontweight="bold")
    fig_text_mm(fig, T_X, B_ROW2 + 7.6,
                "risk must order error positively", fontsize=FS_TINY,
    ...
    for i, rep in enumerate(REP_ORDER):
        ...
        fig_text_mm(fig, T_X + 26.0, yy, "pass" if ok else "fail",
                    fontsize=FS_SMALL, color=PASS_GREEN if ok else STOP_RED,
                    ha="left", va="baseline", fontweight="bold")
    fig_text_mm(fig, T_X, B_ROW2 + 48.0,
                _wrap_mm(fig,
                         "Only the moments encoding ranked its own errors; "
                         "centers and intervals were held.",
                         B_TAB_W, FS_TINY),
