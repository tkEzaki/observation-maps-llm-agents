...
def _shared_key(fig) -> None:
    ...
    fig_text_mm(fig, COL_X[0], KEY_Y, "representations:", ha="left",
...
def build(_out_dir: Path | None = None) -> Path:
    ...
    for panel, m, col, letter, title in (
        ...
        axP.text(-0.36, -148.0,
                 f"{n_gate}/{len(cx)} cells pass the frozen gate",
    ...
    si_note(
        fig,
        f"Two independent seed blocks, {n_rec:,} valid responses, "
        f"{len(kap)} frozen concentrations, {len(REP_ORDER)} observation "
        f"representations; Fourier order $M$ = {ana['primary_fourier_order']}.\n"
        f"Frozen block-to-block trajectory correlations "
        f"$r(a_1)$ = {min(r1):.3f}–{max(r1):.3f} and "
        f"$r(a_2)$ = {min(r2):.3f}–{max(r2):.3f}, all above the frozen "
        f"{proto['replication_rules']['minimum_trajectory_correlation']:g} "
        f"threshold;\n{n_incl}/{n_coef} block-difference intervals for "
        "$a_0$, $a_1$, $a_2$ include zero; error bars are 95% bootstrap "
        "intervals. Frozen phenotype rules are trajectory-level and read the\n"
        f"$\\kappa$ = {min(kap):g} and $\\kappa$ = {max(kap):g} endpoints only. "
        "Bootstrap ellipses and $\\Delta C_1$ origin exclusions are conditional "
        "on the two acquisition blocks, auxiliary and not family-wise adjusted.",
    )
