...
* b  sustained-lock window sweep - lock fraction against the length ``W`` of
  the trailing window over which ``r1`` must stay above threshold;
* c  first passage versus sustained onset - whether a crossing of 0.9 is
...
ENDPOINTS = [
    ...
    ("delta_sustained_lock", "sustained\nlock"),
...
def build(_out_dir: Path | None = None) -> Path:
    ...
    # =====================================================================
    # b  sustained-lock window sweep
    # =====================================================================
    axB = mm_axes(fig, R1_LEFT[1], R1_TOP, R1_W, R1_H)
    panel_label(axB, "b", "Sustained-window sweep", dx_mm=LETTER_DX)
    ...
    # =====================================================================
    # c  first passage versus sustained onset
    # =====================================================================
    ...
    axC.text(2, 128.0, "transient crossing, never sustained",
    ...
    axC.set_ylabel(r"sustained-lock onset ($t$)")
    ...
    axD.text(0.5, -0.20,
             r"$K>0$ only, $n=12$ runs per representation" "\n"
