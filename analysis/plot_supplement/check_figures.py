...
Run:  py -m analysis.plot_supplement.check_figures
"""
...
from analysis.plot_supplement.style import FIG_DIR  # noqa: E402
...
def check_text() -> list[str]:
    ...
    bad: list[str] = []
    from analysis.plot_supplement import style as sist

    real = sist.save_fig
    for n in range(1, 24):
        name = f"figS{n:02d}"
        try:
            mod = importlib.import_module(f"analysis.plot_supplement.{name}")
