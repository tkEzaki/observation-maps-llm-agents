...
def _facts() -> dict:
    ...
    timeline = [
        ...
        (_mtime_utc(FIELDS_JSON), f"{len(fields)} fields locked",
         f"+ {prompts['n_target_prompts']} presented prompts"),
...
def _panel_i(fig, F) -> None:
    ...
    ax.text(I_W - 1.0, y_rule + 1.5, "presented-encoding acquisition below this line",
            ha="right", va="center", fontsize=FS_TINY, color=PASS_GREEN)

    for k, ((when, label, detail), y) in enumerate(zip(events, ys), start=1):
        ax.plot([1.3], [y], marker="o", ms=MS_POINT, color=INK, zorder=3)
        ax.text(3.2, y, f"{k}.", ha="left", va="center",
                fontsize=FS_TINY, color=MUTED)
        ax.text(8.0, y, label, ha="left", va="center", fontsize=FS_TINY,
                color=INK)
        ax.text(8.0, y + 2.5, detail, ha="left", va="center",
                fontsize=FS_TINY, color=MUTED)
    ax.text(0.0, I_H - 0.5,
            "order as recorded; timestamps in the audit",
            ha="left", va="center", fontsize=FS_TINY, color=MUTED)
    panel_label_at(fig, LET3, LET_Y3, "i", "Recorded order of events")
