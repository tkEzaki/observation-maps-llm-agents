...
* the frozen prespecified replication gates for all three families (a);
...
``analysis/figure_data/data/r1_fieldwise_mean_tv.csv``. The per-field and
...
GATE_LABEL = {
    "global_target_effect_p_lt_0.05": r"Presented-encoding effect, $p<0.05$",
    ...
    "invalid_rate_ok": "Valid rate and encoding skew in gate",
}
...
def _panel_a(fig, dec: dict) -> None:
    ...
    ax.text(0.0, 2.0, "Prespecified R1 gate", ha="left", va="center",
...
def _panel_b(fig, dec: dict, stats: dict) -> None:
    ...
    panel_label(ax, "b", "Validity by presented encoding")
...
def _panel_c(fig, stats: dict) -> None:
    for fi, fam in enumerate(FAMS):
        ...
        if fi == 0:
            ...
            panel_label(ax, "c", "Response trinomials by presented encoding",
                        dy_mm=6.2)
        else:
            ax.set_yticklabels([])
