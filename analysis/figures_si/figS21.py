...
b  shared-seed terminal-lock score L, same structure
...
def _panel_b(fig) -> dict:
    ...
    _rep_facets(
        ...
        r"terminal-lock score $L$", "b", "Shared-seed lock score",
...
def _panel_c(fig, y: dict) -> np.ndarray:
    ...
    panel_label(ax, "c", "Family × encoding map")
...
def _panel_i(fig) -> None:
    ...
    ax.add_patch(mpatches.Rectangle((0, 0), 1, 1, transform=ax.transAxes,
                                    ...
                                    lw=LW_THIN, zorder=0))
    items = (
        ("Macro encoding dependence", "established in GPT and Claude",
         VERDICT["supported"]),
        ("Encoding-to-phenotype map", "family-specific", INK),
        ("Microscopic operator dependence",
         "established in GPT, Claude and Gemini", VERDICT["supported"]),
        ("Universal encoding-specific\nphase diagram", "not supported",
