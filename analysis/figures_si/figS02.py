...
``analysis/figures_main/fig2_micro._shared_field_records`` - exact
...
def _draw_page_b(examples, field, replay, stim, gpt, r2, r1) -> Path:
    ...
    _sub_head(
        fig, E_HEAD1_Y, "Encoding-level constants",
    ...
    axR.text(0.0, 2.6, "Retry audit", ha="left", va="baseline",
             fontsize=FS_TINY, fontweight="bold", color=INK)
    axR.text(13.0, 2.6, "complete parser and retry dispositions: "
             "Supplementary Table S2", ha="left", va="baseline",
             fontsize=FS_TINY, color=MUTED)
    head_x = (42.0, 53.0)
    ...
    SCOPES = {
        "OpenAI": "collective runs",
        "Anthropic": "replay + macro",
        "Google": "replay",
    }
    axR.text(0.0, 20.4, "Claude scope excludes the $K$=0 control.",
             ha="left", va="baseline", fontsize=FS_TINY, color=MUTED)
    for i, (model, vendor) in enumerate(PROVIDERS):
        a = audits[vendor]
        y = 10.0 + i * 3.2
        axR.text(0.0, y, f"{model} ({SCOPES[vendor]})", ha="left", va="baseline",
                 fontsize=FS_TINY, color=INK)
