"""publication revision: existing data only; no provider calls.

Run from the repository root: python -m analysis.publication.build_figures
...
def fig1(seeds, series):
    from analysis.plot_main.fig2_micro import _shared_field_records, _excerpt_lines
...
def fig2():
    ...
    from analysis.publication.integrate_claude_figures import fig2 as comparison
    comparison()
...
if __name__=='__main__':
    ...
    for f in ['fig3_replay_operator.pdf','fig4_cross_family.pdf','fig5_observation_map_control.pdf']:
        shutil.copyfile(ROOT/'figures/source_main'/f,OUT/f)
