...
Run:  py -m analysis.natcomm_si_opus.check_figures
"""
...
from analysis.natcomm_si_opus.style import FIG_DIR  # noqa: E402
...
def check_text() -> list[str]:
    ...
    bad: list[str] = []
    from analysis.natcomm_si_opus import style as sist

    real = sist.save_fig
    for n in range(1, 24):
        name = f"figS{n:02d}"
        try:
            mod = importlib.import_module(f"analysis.natcomm_si_opus.{name}")
