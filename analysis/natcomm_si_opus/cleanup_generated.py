...
never anything that is not reproducible from `build_all`.
    ...
    py -m analysis.natcomm_si_opus.cleanup_generated            # scratch only
    py -m analysis.natcomm_si_opus.cleanup_generated --pages    # + built pages
    py -m analysis.natcomm_si_opus.cleanup_generated --dry-run
"""
...
from analysis.natcomm_figures_opus.style import ROOT

FIG_DIRS = [
    ROOT / "figures" / "natcomm_opus",
    ROOT / "figures" / "natcomm_si_opus",
]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.dry_run:
        ...
        print("\nrebuild with: py -m analysis.natcomm_figures_opus.build_all"
              " && py -m analysis.natcomm_si_opus.build_all")
    return 0
