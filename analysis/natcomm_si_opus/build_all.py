...
crashing the run.
    ...
    py -m analysis.natcomm_si_opus.build_all            # all
    py -m analysis.natcomm_si_opus.build_all 12 13 20   # a subset
"""
...
from analysis.natcomm_si_opus.style import FIG_DIR

ORDER = [f"figS{n:02d}" for n in range(1, 24)]


def _build_one(mod_name: str):
    mod = importlib.import_module(f"analysis.natcomm_si_opus.{mod_name}")
    return mod.build()
...
def main(argv: list[str] | None = None) -> int:
    ...
    for mod_name in wanted:
        try:
            importlib.import_module(f"analysis.natcomm_si_opus.{mod_name}")
    ...
    manifest = {
        "variant": "si_opus",
        "plan": "docs/NATCOMM_SI_FIGURE_PANEL_PLAN_DETAILED.md",
        "width_mm": 180.0,
        "png_dpi": 450,
        "pdf_fonttype": 42,
        "built": built,
