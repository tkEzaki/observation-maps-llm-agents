...
def build(_out_dir: Path | None = None) -> Path:
    ...
    variants = [
        ...
        ("reformatted", OMAP["moments_reformatted"], "table layout of same $\tilde z_k$"),
        ("task-irrelevant padded", OMAP["moments_length_matched"], "same $\tilde z_k$ + task-irrelevant context"),
    ]
    box_h = 4.6
    ...
    axD.hist(null_tv, bins=40, color="#C9CDD2", edgecolor="white", linewidth=0.2,
             density=True)
    ...
    top = axD.get_ylim()[1]
    axD.text(null_meta["null_mean"] + 0.007, top * 0.99, "null mean",
