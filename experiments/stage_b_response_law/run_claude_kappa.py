...
from circlemap.response import parse_social_action
from experiments.stage_b_response_law.run_representation import build_tasks

ROOT = Path(__file__).resolve().parents[2]
DEFAULT_SPEC = Path(__file__).with_name('protocol_claude_kappa_v1.json')
...
def main(argv=None):
    ...
    root = (args.run_root or ROOT/'runs/claude_kappa_v1'/backend_name).resolve()
