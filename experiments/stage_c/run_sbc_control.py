"""Serialization-binding ladder runner (48 fields x 4 conditions x 16 responses).

Uses the frozen physical fields of the identical-field replay. All four
conditions carry the same 24 bin masses at the same precision; they differ only
in how each mass is bound to its bin, and the two anchor conditions are byte
identical to those already acquired in the serialization-length control.
"""
...
DEFAULT_PROTOCOL = Path(__file__).with_name(
    "protocol_matched_rep_collective_sbc_v0_1.json"
)
AUTH_PATH = ROOT / "analysis/matched_rep_collective/sbc_auth_go.json"
...
def build_tasks(protocol: dict, fields: list[dict]) -> list[SbcTask]:
    n_blocks = int(protocol["n_blocks"])
    per_block = int(protocol["responses_per_block"])
    variants = list(protocol["control_variants"])
    assert tuple(variants) == SBC_VARIANTS
    seed_prefix = protocol.get("seed_prefix", "mrc-sbc-v0.1")
    ...
    for field in fields:
        ...
        for variant in variants:
            prompt = build_sbc_prompt(variant, hist)
            ph = _sha_text(prompt)
...
def audit_tasks(tasks: list[SbcTask], protocol: dict) -> dict:
...
def require_offline_audit() -> dict:
    ...
    if not AUDIT_PATH.exists():
        raise SystemExit(
            "SBC_AUDIT_MISSING: run "
            "analysis/matched_rep_collective/verify_sbc_information_equivalence.py"
        )
    audit = json.loads(AUDIT_PATH.read_text(encoding="utf-8"))
    if not audit.get("pass"):
        raise SystemExit("SBC_AUDIT_FAIL")
    eq = audit["information_equivalence"]
    if eq["max_abs_bin_mass_gap"] != 0.0:
        raise SystemExit("SBC_AUDIT_INFORMATION_NOT_IDENTICAL")
    if not all(audit["anchors_byte_identical"].values()):
        raise SystemExit("SBC_AUDIT_ANCHORS_DRIFTED")
    if not audit["length_axis_crosses_binding_axis"]["crossed"]:
        raise SystemExit("SBC_AUDIT_CROSSING_LOST")
    return audit
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-root",
        type=Path,
        default=ROOT / "runs" / "matched_rep_collective_sbc",
    )
    ...
    print(
        format_cost_estimate(estimate).replace(
            "Stage B", "Serialization-binding ladder"
        ),
        flush=True,
    )
    ...
    if not AUTH_PATH.exists():
        raise SystemExit("SBC_PAID_AUTH_MISSING")
    auth = json.loads(AUTH_PATH.read_text(encoding="utf-8"))
    if not auth.get("paid_authorized"):
        raise SystemExit("SBC_PAID_AUTH_FALSE")
    if not args.yes:
        raise SystemExit("Refusing paid serialization-binding run without --yes")
