"""Same-information observation-map control runner (48×3×16).

Uses frozen physical fields; variants change only moments serialization/length.
"""
...
DEFAULT_PROTOCOL = Path(__file__).with_name(
    "protocol_matched_rep_collective_omap_control_v0_1.json"
)
AUTH_PATH = ROOT / "analysis/matched_rep_collective/omap_auth_go.json"
...
def build_tasks(protocol: dict, fields: list[dict]) -> list[OmapTask]:
    n_blocks = int(protocol["n_blocks"])
    per_block = int(protocol["responses_per_block"])
    variants = list(protocol["control_variants"])
    assert tuple(variants) == CONTROL_VARIANTS
    seed_prefix = protocol.get("seed_prefix", "mrc-omap-v0.1")
...
def audit_tasks(tasks: list[OmapTask], protocol: dict) -> dict:
    ...
    checks = {
        ...
        "n_variants_3": len({t.target_representation for t in tasks}) == 3,
        ...
        and len(per_cond) == 48 * 3 * n_blocks,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-root",
        type=Path,
        default=ROOT / "runs" / "matched_rep_collective_omap",
    )
    ...
    print(
        format_cost_estimate(estimate).replace("Stage B", "Observation-map control"),
        flush=True,
    )
    ...
    if not auth.get("paid_authorized"):
        raise SystemExit("OMAP_PAID_AUTH_FALSE")
    if not args.yes:
        raise SystemExit("Refusing paid omap run without --yes")
    ...
    if args.resume_dir is not None:
        ...
        run_dir = (
            args.out_root
            / f"{protocol['protocol_version']}_{args.model}"
            / stamp
        )
    ...
    # run_one_task expects attributes matching ReplayTask — OmapTask is compatible
